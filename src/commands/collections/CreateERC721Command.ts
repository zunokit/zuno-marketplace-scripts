...
export class CreateERC721Command extends BaseCommand {
  ...
  async execute(
    ...
    try {
      ...
      // Get factory ABI
      const factoryABI = await context.abiProvider.getABI(
      ...
      const collectionParams = formatCollectionParams(
        {
          ...
          description:
            args?.description || "A test ERC721 collection with full features",
          ...
          tokenURI:
            args?.tokenURI || "https://api.example.com/erc721/metadata/",
          ...args,
        },
        provider.account
      );
      ...
      logger.info(
        `Mint Price: ${ethers.formatEther(collectionParams.mintPrice)} ETH`
      );
      ...
      if (!collectionAddress) {
        throw new Error(
          "Could not find collection address in transaction logs"
        );
      }
  ...
  private async verifyCollection(
    ...
    try {
      ...
      if (!name || !symbol) {
        ...
        logger.warning(
          "This might be a bug in the smart contract initialization."
        );
      }
    } catch (error) {
